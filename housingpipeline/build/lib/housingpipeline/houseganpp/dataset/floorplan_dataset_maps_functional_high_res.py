...
def filter_graphs(graphs):
    ...
    for graph in graphs:
        ...
        # check that num ext walls == num corner edges
        num_ext_walls = len([n for n in room_type_and_features if n[0] == 1])
        num_corner_type_edges = 0
        for edge in edges:
            if (
        ...
        check_none = np.sum([bb is None for bb in room_bbs])
        if (
...
# class FloorplanGraphDataset_RP(Dataset):
...
class FloorplanGraphDataset(Dataset):
    def __init__(self, shapes_path, transform=None, target_set="D", split="train"):
        ...
        # Potentially, if HG and HG++ use the same format for mks, nds, eds, we could use this dataset in HG++?
        # Or maybe we modify it some to make it work.
        # And then modify the dataset and HG++ to accept more node/edge features, constructed from LIFULL
        # And then train.
        super(Dataset, self).__init__()
        self.shapes_size = 1.0 # or 256.0 if data does not scale
        ...
        for graph in self.subgraphs:
            room_bbs = graph[0]
            in_set = (len(room_bbs) >= min_N) and (len(room_bbs) <= max_N)
            if (split == "train") and (in_set == False):
                split_subgraphs.append(graph)
    ...
    def __getitem__(self, index):
        ...
        rooms_mks = torch.FloatTensor(rooms_mks)
        rooms_mks = self.transform(rooms_mks)
        nodes = torch.FloatTensor(room_type_and_features)
        edges = torch.LongTensor(edges)
        edges_features = torch.FloatTensor(edges_features)

        return rooms_mks, nodes, edges, edges_features
...
def floorplan_collate_fn(batch):
    all_rooms_mks, all_nodes, all_edges, all_edges_features = [], [], [], []
    all_node_to_sample, all_edge_to_sample = [], []
    node_offset = 0
    eds_sets = []
    for i, (rooms_mks, nodes, edges, edges_features) in enumerate(batch):
        O, T = nodes.size(0), edges.size(0)
        all_rooms_mks.append(rooms_mks)
        all_nodes.append(nodes)
        all_edges_features.append(edges_features)
    ...
    all_edges_features = torch.cat(all_edges_features)
    if len(all_edges) > 0:
