...
def selectNodesTypes(nd_to_sample, batch_size, nds):
    ...
    for k in range(batch_size):
        # this line returns indices of nodes within a single batch sample
        rooms = np.where(nd_to_sample == k)
        # calc number of rooms
        rooms_num = np.array(rooms).shape[-1]
        ...
        # okay, "fixed" means a part of random type-selection
        fixed_nodes_state = torch.tensor(fixed_rooms).cuda()
        # add shift because a batch is actually all samples stacked, so this way we keep track of room ids per sample
        fixed_nodes_state += shift
        fixed_nodes.append(fixed_nodes_state)
        shift += rooms_num
    # I guess we can concatenate a list of tensors, dimensions are kept, stacking in zeroth dim
    fixed_nodes = torch.cat(fixed_nodes)
    bin_fixed_nodes = torch.zeros((nd_to_sample.shape[0], 1))
...
def draw_masks(masks, real_nodes, im_size=256):
    ...
    for m, nd in zip(masks, real_nodes):
        ...
        # pick color
        nd_id = np.nonzero(nd)[0][0] + 1
        color = ID_COLOR[nd_id]
